...
describe('leer avisos, sin token', function () {
    ...
    test('cada aviso trae los campos del PostResource', function () {
        Post::factory()->create([
            'categoria_id' => $this->categoria->id,
...
describe('la Policy de la API', function () {
    test('editar un aviso ajeno responde 403', function () {
        ...
        expect($ajeno->fresh()->titulo)
            ->not->toBe('Secuestrado');
    });

    test('editar el propio aviso responde 200', function () {
        ...
        $this->putJson("/api/avisos/{$mio->id}", [
            ...
            ->assertStatus(200)
            ->assertJsonPath('data.titulo', 'Corregido');
    });

    test('un admin puede eliminar un aviso ajeno', function () {
        $ajeno = Post::factory()->create([
            'categoria_id' => $this->categoria->id,
        ]);
        ...
        $this->deleteJson("/api/avisos/{$ajeno->id}")
            ->assertStatus(204);
